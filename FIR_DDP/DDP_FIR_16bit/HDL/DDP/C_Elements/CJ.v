//`timescale 1ns/ 100ns
(* dont_touch = "true" *)
module CJ #(
    parameter delay_LA = 10,
    parameter delay_LB = 10,
    parameter delay_LC = 10,
    parameter delay_LD = 10
)(
    /*** port ***/
    input MR,
    input z, g,
    input Send_in,
    input Ack_in,
    output Send_out,
    output Ack_out,
    output cp
);

    (* dont_touch = "true" *) wire nand1_out, nand2_out, nand3_out, nand4_out, nand5_out;
    (* dont_touch = "true" *) wire LA_out, LB_out, LC_out, LD_out;

    assign nand1_out = ~(Send_in & nand2_out);
    assign nand2_out = ~(nand1_out & ~MR & nand3_out); 
    assign nand3_out = ~(z & g & Send_in & LA_out & Ack_in & LC_out);
    assign nand4_out = ~(nand3_out & nand5_out);
    assign nand5_out = ~(nand4_out & ~MR & Ack_in);

    Delay #(.delay_num(delay_LA)) delay1 (.din(nand1_out), .dout(LA_out));
    Delay #(.delay_num(delay_LB)) delay2 (.din(nand1_out), .dout(LB_out));
    Delay #(.delay_num(delay_LC)) delay3 (.din(nand5_out), .dout(LC_out));
    Delay #(.delay_num(delay_LD)) delay4 (.din(LC_out), .dout(LD_out));

    //output
    assign Send_out = ~LD_out;
    assign Ack_out = ~LB_out;
    assign cp = ~nand5_out;
endmodule