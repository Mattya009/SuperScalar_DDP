...
module CM #(
    ...
    parameter delay_LB = 10,
    parameter delay_LC = 10,
    parameter delay_LD = 10
)(
    /*** port ***/
    input MR,
    input Send_in_a,
    ...
    /*** module ***/
    CJ #(.delay_LA(delay_LA), .delay_LB(delay_LB), .delay_LC(delay_LC), .delay_LD(delay_LD)) CJ1 (.Send_in(Send_in_a), .z(z2), .g(nand8_out), .Ack_out(Ack_out_a), .MR(MR), .Send_out(CJ1_Send_out), .Ack_in(Ack_in), .cp(cp1));
    CJ #(.delay_LA(delay_LA), .delay_LB(delay_LB), .delay_LC(delay_LC), .delay_LD(delay_LD)) CJ2 (.Send_in(Send_in_b), .z(z1), .g(nand7_out), .Ack_out(Ack_out_b), .MR(MR), .Send_out(CJ2_Send_out), .Ack_in(Ack_in), .cp(cp2));
endmodule