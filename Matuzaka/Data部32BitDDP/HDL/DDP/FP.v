...
module FP(
    ...
    (* dont_touch = "true" *) wire [`Data_LENGTH-1:0] FLOW_DATA;
    ...
    assign {DataL, DataR} = DL[63:0];
    assign {color, gen, dest, LR2, BR, CPY, OPC, C, Z} = DL[93:64];
    assign {FLOW_DATA, WRITE_EN, WRITE_DATA, PACKET_OUT, LOAD_FLG} = ALU(color, gen, dest, LR2, BR, CPY, OPC, C, Z, DataL, DataR);
    ...
    function [`ALU_LENGTH-1:0] ALU;
        ...
        case (OPC)
            `ADD:       begin
                            ...
                            {ALU[`ALU_FLOW_DATA], ALU[`ALU_PACKET_ResultData]} = ($signed(DataL) + $signed(DataR));
                            ALU[`ALU_C] = |ALU[`ALU_FLOW_DATA];
                            ALU[`ALU_Z] = ~|ALU[`ALU_PACKET_ResultData];
                        end
            `ADDC:      begin
                            {ALU[`ALU_WRITE_EN], ALU[`ALU_WRITE_DATA], ALU[`ALU_LOAD_FLG]} = {1'b0, DataL, 1'b0};
                            ALU[`ALU_color_to_CPY] = {color, gen, dest, LR2, BR, CPY};
                            {ALU[`ALU_FLOW_DATA], ALU[`ALU_PACKET_ResultData]} = ($signed(DataL) + $signed(DataR) + $signed(C));
                            ALU[`ALU_C] = |ALU[`ALU_FLOW_DATA];
                            ALU[`ALU_Z] = ~|ALU[`ALU_PACKET_ResultData];
                        end
            `SUB:       begin
                            {ALU[`ALU_WRITE_EN], ALU[`ALU_WRITE_DATA], ALU[`ALU_LOAD_FLG]} = {1'b0, DataL, 1'b0};
                            ALU[`ALU_color_to_CPY] = {color, gen, dest, LR2, BR, CPY};
                            ALU[`ALU_PACKET_ResultData] = $signed(DataL) - $signed(DataR);
            ...
            `SUBC:      begin
                            {ALU[`ALU_WRITE_EN], ALU[`ALU_WRITE_DATA], ALU[`ALU_LOAD_FLG]} = {1'b0, DataL, 1'b0};
                            ALU[`ALU_color_to_CPY] = {color, gen, dest, LR2, BR, CPY};
                            ALU[`ALU_PACKET_ResultData] = $signed(DataL) - $signed(DataR) - $signed(C);
                            ALU[`ALU_C] = (DataL < DataR)? 1'b1 : 1'b0;
                            ALU[`ALU_Z] = ~|ALU[`ALU_PACKET_ResultData];
                        end
            `AND:       begin
            ...
            `MUL:       begin
                            {ALU[`ALU_WRITE_EN], ALU[`ALU_WRITE_DATA], ALU[`ALU_LOAD_FLG]} = {1'b0, DataL, 1'b0};
                            ALU[`ALU_color_to_CPY] = {color, gen, dest, LR2, BR, CPY};
                            {ALU[`ALU_FLOW_DATA],ALU[`ALU_PACKET_ResultData]} = ($signed(DataL) * $signed(DataR)>>>32);
                            ALU[`ALU_C] = |ALU[`ALU_FLOW_DATA];
                            ALU[`ALU_Z] = ~|ALU[`ALU_PACKET_ResultData];
                        end
            `SHL:       begin
                            {ALU[`ALU_WRITE_EN], ALU[`ALU_WRITE_DATA], ALU[`ALU_LOAD_FLG]} = {1'b0, DataL, 1'b0};
                            ALU[`ALU_color_to_CPY] = {color, gen, dest, LR2, BR, CPY};
                            {ALU[`ALU_FLOW_DATA], ALU[`ALU_PACKET_ResultData]} = DataL << DataR;
                            ALU[`ALU_C] = |ALU[`ALU_FLOW_DATA];
                            ALU[`ALU_Z] = ~|ALU[`ALU_PACKET_ResultData];
                        end
            `SHR:       begin
            ...
            `ADDGEN:    begin
                            {ALU[`ALU_WRITE_EN], ALU[`ALU_WRITE_DATA], ALU[`ALU_LOAD_FLG]} = {1'b0, DataL, 1'b0};
                            ALU[`ALU_color_to_CPY] = {color, (($signed(gen) + $signed(DataR[`gen_LENGTH-1:0])) <= `gen_max)? $signed(gen) + $signed(DataR[`gen_LENGTH-1:0]) : `gen_reset, dest, LR2, BR, CPY};
                            ALU[`ALU_PACKET_ResultData] = DataL;
                            ALU[`ALU_C] = C;
                            ALU[`ALU_Z] = Z;
                        end
