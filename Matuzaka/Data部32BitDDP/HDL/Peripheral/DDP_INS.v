...
module DDP_INS (
    ...
    always @ (posedge Send or posedge MR) begin
        if(MR) begin
           addr <= 8'd0;
            ...
            addr <= 8'd13;  //リセット。IMEM0~11は０がパディングしてある
        end
        else begin
            addr <= addr + 8'd1;
        end
    ...
    //IMEM EN(Send)の立ち上がりで動作�?�カウンタのアドレスでメモリにアクセスして、IMEM?��DOUTとして出�?
    IMEM imem (
    ...
    assign IMEM_DOUT = (enable)?{3'd0, addr-8'd13, 7'd0, 4'd0, IMEM_out}: {3'd0, 8'd0, dest, 4'b1100, 32'd0};
endmodule
